...
class ControllerExtensionPaymentPlugLiqPay extends Controller {
	public function index() {
		...
		$data['signature'] = base64_encode(sha1($this->config->get('plug_liqpay_signature') . $xml . $this->config->get('plug_liqpay_signature'), true));

		return $this->load->view('extension/payment/plug_liqpay', $data);
	}

	public function confirm() {
		if ($this->session->data['payment_method']['code'] == 'plug_liqpay') {
			$this->load->model('checkout/order');

			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('plug_liqpay_order_status_id'));
		}
	}

	public function callback() {
		$xml = base64_decode($this->request->post['operation_xml']);
		$signature = base64_encode(sha1($this->config->get('plug_liqpay_signature') . $xml . $this->config->get('plug_liqpay_signature'), true));
		...
		if ($signature == $this->request->post['signature']) {
			$this->load->model('checkout/order');
			if ($p->status == "success") {
				$status_id = $this->config->get('plug_liqpay_order_success_status_id');
			} elseif ($p->status == "reversed") {
				$status_id = $this->config->get('plug_liqpay_order_reversed_status_id');
			} elseif ($p->status == "processing") {
				$status_id = $this->config->get('plug_liqpay_order_processing_status_id');
			} elseif ($p->status == "failure") {
				$status_id = $this->config->get('plug_liqpay_order_failure_status_id');
			}
