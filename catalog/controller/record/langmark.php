	...
	public function index() {
			...
			if ($count_languages > 1) {
				foreach ($results as $result) {
					...
					/// Robinzone Style Begin
                $languages = $this->data['languages'];
                foreach ($languages as $index => $result) {

                        $languages[$index]['name'] =  $this->getLanguageName($result['code']);

                        if($result['code'] == $this->data['code']) {
                            $current_lang_name = $this->getLanguageName($result['code']);
                        }
                ...
                if (count($languages) > 1) {
                    $this->data['current_lang_name'] = $current_lang_name;
                }
                /// Robinzone Style End
                ///
