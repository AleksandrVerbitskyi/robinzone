...
class ControllerAccountWishList extends Controller {
	public function index() {
		...
		$results = $this->model_account_wishlist->getWishlist(($page - 1) * 5, 5);
                ...
                foreach ($this->model_catalog_product->getProductOptions($product_info['product_id']) as $option) {
                    if ($option['type'] != 'file') {
                        ...
                        foreach ($option['product_option_value'] as $option_value) {
                            array_push($value, $option_value['name']);
                        }
	...
	public function add() {
		...
		if ($product_info) {
			if ($this->customer->isLogged()) {
				...
				$json['success'] = sprintf($this->language->get('text_login'), $this->url->link('account/login', '', true), $this->url->link('account/login', '', true), $this->url->link('product/product', 'product_id=' . (int)$this->request->post['product_id']), $product_info['name'], $this->url->link('account/wishlist'));
				$json['warning'] = '1';
