...
class ControllerAccountDownload extends Controller {
	public function index() {
		...
		$download_total = $this->model_account_download->getAllTotalDownloads();

		$results = $this->model_account_download->getAllDownloads(($page - 1) * $this->config->get($this->config->get('config_theme') . '_product_limit'), $this->config->get($this->config->get('config_theme') . '_product_limit'));
		...
		foreach ($results as $result) {
			if (file_exists(DIR_DOWNLOAD . $result['filename'])) {
				...
				$data['downloads'][] = array(
					...
					'image'       => $this->getDownloadImage($result['mask']),
...
//		$data['column_right'] = $this->load->controller('common/column_right');
	...
	private function getDownloadImage($filename) {
	    $path = DIR_TEMPLATE . 'robinzone/img/filetypes/';
	    $image_name = strtolower(substr(strrchr($filename, '.'), 1)) . '.png';
	    if (file_exists($path . $image_name)) {
	        return 'catalog/view/theme/robinzone/img/filetypes/' . $image_name;
        }
        return '';
    }
	...
	public function download() {
		...
		$download_info = $this->model_account_download->getFileDownload($download_id);
