...
class ControllerAccountReturn extends Controller {
	...
	public function index() {
		...
		foreach ($results as $result) {
		    $reason = $this->model_account_return->getReturn($result['return_id'])['reason'];
			$data['returns'][] = array(
		...
		foreach ($data['returns'] as $key => $return) {
            $product_info = $this->model_catalog_product->getProduct($return['product_id']);
            ...
            if ($product_info) {
                ...
                $options = [];
                $options_tmp = $this->model_account_order->getOrderOptionsByProduct($return['order_id'], $product_id);
                ...
                $data['returns'][$key]['product'] = $product;
                $data['returns'][$key]['summ'] = $this->currency->format($product_info['price'] * $return['quantity'], $this->session->data['currency']);
            }
        }
...
//		$data['column_right'] = $this->load->controller('common/column_right');
	...
	public function info() {
		...
		if ($return_info) {
            ...
            $this->document->setBreadcrumbs($data['breadcrumbs']);
            $data['text_greeting'] = sprintf($this->language->get('text_greeting'), $this->customer->getFullName());
            $data['address_url'] = $this->url->link('account/return', '', true);
...
//			$data['column_right'] = $this->load->controller('common/column_right');
	...
	public function add() {
		...
		$this->document->addScript('catalog/view/javascript/jquery/jquery.datetimepicker.full.js');
		$this->document->addStyle('catalog/view/javascript/jquery/jquery.datetimepicker.css');
		...
		$data['entry_fault_detail2'] = $this->language->get('entry_fault_detail2');
        ...
        $this->document->setBreadcrumbs($data['breadcrumbs']);
        $data['text_greeting'] = sprintf($this->language->get('text_greeting'), $this->customer->getFullName());
        $data['address_url'] = $this->url->link('account/address', '', true);
