...
class ControllerProductCompare extends Controller {
    ...
    private $filters_to_show = ['Сезон', 'Сезон']; // Must write for all languages
	...
	public function index() {
		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
                ...
                foreach ($this->model_catalog_product->getProductFilters($product_id) as $group_name => $filter_items) {
                    $filters[$group_name] = [];
                    foreach ($filter_items as $value) {
                        $filters[$group_name][] = $value;
                    }
                }

				$options = [];

				foreach ($this->model_catalog_product->getProductOptions($product_id) as $index => $options_item) {
                    $options[$options_item['name']] = [];
                    foreach ($options_item['product_option_value'] as $value) {
                        $options[$options_item['name']][] = $value['name'];
                    }
                }
		...
		$data['all_options'] = $this->getAllOptions($data['products']);
		$data['all_filters'] = $this->getAllFilters($data['products']);
	...
	private function getAllOptions($products) {
	    $result = [];
	    foreach ($products as $product) {
	        if (isset($product['options'])) {
	            foreach ($product['options'] as $name => $values) {
	                if (!in_array($name, $result)) array_push($result, $name);
                }
            }
        }
        return $result;
    ...
    private function getAllFilters($products) {
        $result = [];
        foreach ($products as $product) {
            if (isset($product['filters'])) {
                foreach ($product['filters'] as $group => $values) {
                    if (!in_array($group, $result) && in_array($group, $this->filters_to_show)) array_push($result, $group);
                }
            }
        }
        return $result;
