...
class ControllerCommonCart extends Controller {
	public function index() {
        ...
        $data['cart_icon'] = 'catalog/view/theme/robinzone/img/basket.jpg';
		...
		$data['all_in_cart'] = $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0);

		$data['cart'] = $this->url->link('checkout/cart');
		$data['checkout'] = $this->url->link('checkout/cart', '', true);

		if ($this->isAjax()) {
		    if (isset($this->request->post['action']) && $this->request->post['action'] == 'getProductsItems') {
		        $json = [];
		        $json['products'] = $data['products'];
		        $json['totals'] = $data['totals'];
		        $json['checkout'] = $data['checkout'];
		        $json['text_checkout'] = $data['text_checkout'];
