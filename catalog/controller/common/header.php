...
//        $this->load->model('extension/module/database_schema_changing');
//        $this->model_extension_module_database_schema_changing->execute();

		$data['analytics'] = array();

		if (!isset($this->session->data['customer_id'])) {
		    if (isset($_COOKIE['SELECTOR']) && isset($_COOKIE['TOKEN'])) {
                $selector = $_COOKIE['SELECTOR'];
                $token = $_COOKIE['TOKEN'];
                $this->customer->checkRememberMe($selector, $token);
            }
        }
        ...
        $data['text_login_logup'] = $this->language->get('text_login_logup');
...
//        $data['register'] = $this->url->link('account/login', '', true);
        ...
        $data['login_logup'] = $this->url->link('account/login', '', true);
		...
		$data['link_about'] = $this->url->link('information/information', 'information_id=4', true);
        $data['link_opt'] = $this->url->link('information/information', 'information_id=7', true);
//        $data['link_store'] = $this->url->link('product/category', 'catrgory_id=88', true);
        $data['link_store'] = $this->url->link('product/category', 'path=' . '60' , true);
        $data['link_representation'] = $this->url->link('information/representatives', '' , true);
        $data['link_contact'] = $this->url->link('information/contact', '', true);
        ...
        $data['isOnProductPage'] = $this->document->getIsOnProductPage();

        $data['text_special'] = $this->language->get('text_sale');
        $data['link_special'] = $this->url->link('product/special', '', true);
