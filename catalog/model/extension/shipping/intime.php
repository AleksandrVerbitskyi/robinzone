...
class ModelExtensionShippingintime extends Model {
    ...
    function getQuote($address) {
        ...
        if ($address == '') {
            $status = true;
        } elseif ($this->config->get('intime_status')) {
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE 
            geo_zone_id = '" . (int) $this->config->get('intime_geo_zone_id') . "' 
        ...
        if ($status) {
            ...
            $cost = 0.00;
            if ($this->config->get('intime_min_total_for_free_intime') > $this->cart->getSubTotal()) {
                $cost = $this->config->get('intime_intime_price');
            }
