...
class ModelExtensionShippingnovaposhta extends Model {
    ...
    function getQuote($address) {
        ...
        if ($address == '') {
            $status = true;
        } elseif ($this->config->get('novaposhta_status')) {
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE 
            geo_zone_id = '" . (int) $this->config->get('novaposhta_geo_zone_id') . "' 
        ...
        if ($status) {
            ...
            $cost = 0.00;
            if ($this->config->get('novaposhta_min_total_for_free_novaposhta') > $this->cart->getSubTotal()) {
                $cost = $this->config->get('novaposhta_novaposhta_price');
            }
