...
class ModelAccountDownload extends Model {
    ...
    public function getFileDownload($download_id) {
        $query = $this->db->query("SELECT d.filename, d.mask FROM `" . DB_PREFIX . "download` d 
    ...
    public function getAllDownloads($start = 0, $limit = 20) {
        if ($start < 0) {
            $start = 0;
        }
        ...
        WHERE dd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND status = 1 
        ORDER BY dd.name ASC LIMIT " . (int)$start . "," . (int)$limit);

        return $query->rows;
    }

	public function getAllTotalDownloads() {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "download` d 
        LEFT JOIN " . DB_PREFIX . "download_description dd ON (d.download_id = dd.download_id) 
        WHERE status = 1 AND dd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
