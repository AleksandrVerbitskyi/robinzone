...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		thirdname = '" . $this->db->escape(isset($data['thirdname']) ? $data['thirdname'] : '') . "', 
		birthday = '" . $this->db->escape(isset($data['birthday']) ? $data['birthday'] : '') . "', 
        ...
        $message .= sprintf($this->language->get('text_login') . "\n", $data['password'], $this->url->link('account/login', '', true));
...
//		}
		...
		$message .= $this->url->link('account/password', '', true) . "\n\n";
		...
		return ['customer_id' => $customer_id, 'tmp_password' => $data['password']];
	}
	...
	public function editCustomer($data) {
		...
		firstname = '" . $this->db->escape($data['firstname']) . "', 
		lastname = '" . $this->db->escape($data['lastname']) . "', 
		thirdname = '" . $this->db->escape($data['thirdname']) . "', 
	...
	public function addLoginAttempt($email) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_login WHERE 
		email = '" . $this->db->escape(utf8_strtolower((string)$email)) . "' 
		...
		if (!$query->num_rows) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "customer_login SET 
			total = (total + 1), date_modified = '" . $this->db->escape(date('Y-m-d H:i:s')) . "' 
