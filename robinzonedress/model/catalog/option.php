		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				if ($option_value['option_value_id']) {
                    $_1s_id = isset($option_value['1s_id']) && !empty($option_value['1s_id']) ? $option_value['1s_id'] : '';
                    $sql = "INSERT INTO " . DB_PREFIX . "option_value SET";
				    $sql .= " option_value_id = '" . (int)$option_value['option_value_id'] . "',";
				    $sql .= " option_id = '" . (int)$option_id . "',";
				    $sql .= " image = '" . $this->db->escape(html_entity_decode($option_value['image'], ENT_QUOTES, 'UTF-8')) . "',";
				    $sql .= " sort_order = '" . (int)$option_value['sort_order'] . "',";
				    $sql .= " 1s_id = '" . $_1s_id . "'";
					$this->db->query($sql);
				} else {
				    $sql = "INSERT INTO " . DB_PREFIX . "option_value SET";
