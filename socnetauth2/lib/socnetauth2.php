...
Class SocAuth extends DB
{
	...
	{	
		$check_code = $this->query_rows("SHOW COLUMNS FROM `".DB_PREFIX."setting` LIKE 'code'");
		$check_group = $this->query_rows("SHOW COLUMNS FROM `".DB_PREFIX."setting` LIKE 'group'");
		
		if( !empty($check_group) )
		{
			$this->group_name = 'group';
		}
		elseif( !empty($check_code) )
		{
			$this->group_name = 'code';
		}
		else
		{
			exit("ERROR grop field");
		}
	}
	...
	function setSessionData($keyword, $value)
	{
		$session_key = '';
